...
6. **Named-class-attribution gate (the sneaky one).** For every complexity or tractability claim, identify its *justification*. If the justification is "P is / reduces to / is an instance of [named problem Q]" or "polynomial by [named technique — transportation, matching, min-cost flow, TU/integral LP, …]", treat it as `BLOCKING` unless the prover supplied all of: (a) an explicit reduction map; (b) a *verified* — not asserted — check that P satisfies Q's defining structural hypotheses (demand the constraint matrix; for transportation/flow/TU confirm no weighted-sum or non-margin side constraints; for matching confirm bipartite + degree-only); (c) an explicit statement of the smallest structural difference between P and the textbook Q and why it doesn't break the reduction. Do not accept "it is well known that Q is polynomial" as evidence that P is. If the claim rests on a polytope being integral/TU, require the corroborating coder vertex-enumeration check (a fractional vertex found → `BLOCKING`); if no such check exists, that absence is itself a `MAJOR` finding. This gate applies even when the claim's *output* was validated against brute force — output correctness does not certify the justification.
