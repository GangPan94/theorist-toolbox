...
| [`workstreams/`](workstreams/) | One directory per workstream (literature, computational, prover, etc.). |
...
2. **Spawn workstreams.** Each sub-goal becomes one (or more) workstream. The
   ...
   (literature-reviewer, prover, coder).
3. **Review and steer.** Read incremental reports in
   `workstreams/W*/report.md`. Intervene at any time by messaging the
   project-coordinator.
4. **Compile the paper.** `paper.tex` is the durable artifact. Margin notes
   record provenance, `\unproven{}` blocks flag every unverified step, and the
   "Open obligations" appendix lists them all.
